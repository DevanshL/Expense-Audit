...
async function generateGeminiSummary(result, dataset, config) {
  const { apiKey, model = 'gemini-2.0-flash', temperature = 0.3, maxTokens = 1024 } = config;
  ...
  logger.debug('Gemini summary request', { model, hasApiKey: !!apiKey });
...
Be specific to this actual analysis data. Do NOT use generic fraud detection advice. Reference the actual MAD score, specific statistical findings, and actual digit deviations observed in this dataset.`;
  ...
  try {
    logger.debug('Gemini API request', { model });
    ...
    logger.debug('Gemini response received', { status: response.status });
    ...
    if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
      logger.error('No content from Gemini API');
...
/**
 * Chat with Gemini — accepts an array of {role, content} messages.
 * Used by ChatService for multi-turn conversations.
 */
